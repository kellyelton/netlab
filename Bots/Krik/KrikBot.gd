...
export (int) var max_energy = 20000 
export (float) var min_speed_percent = 0.3
export (int) var min_time_between_bites_ms = 500
export (String) var bot_type = "KrikBot"
export (bool) var show_debug_lines = false

onready var start_time = OS.get_ticks_msec()
onready var fov_size = get_node("rays").get_child_count()
onready var controller = preload("./KrikController.gd").new(self)
...
func _ready():
	...
	# TODO: Would be cool if these guys had gravity they could control to eat, like a black hole
...
func _physics_process(delta):
	...
	rotation += dir.y * rotation_speed * delta
		...
		if OS.get_ticks_msec() >= (self.time_last_bite + min_time_between_bites_ms):
			for node in self.touching:
				if node.is_in_group("food"):
					...
					var energy_gain = node.bite(self, 900)
...
func _draw():
	...
	for debug_line in self.debug_lines:
		...
		var to = debug_line[1]
		draw_line(from, to, Color.indigo, 1, true)
		
		var rect = Rect2(to, Vector2(1.2, 1.2))
...
func make_baby():
	...
	if OS.get_ticks_msec() >= (1000 * 5) + lastBaby:
		...
		var newBot = load("./KirkBot.tscn").instance()
...
func die(reason):
	...
	print("bot died at age " + str(age) + " (" + str(self.score) + "): " + reason)
...
func _on_KrikBot_input_event(viewport, event, shape_idx):
